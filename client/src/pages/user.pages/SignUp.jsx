...
import { LogIn, Loader, Mail, Lock, User, } from 'lucide-react';
        ...
        if (url === '/auth/register') {
          if (data.name && data.email && data.password) {
             ...
             reject({
              response: { 
                status: 400, 
                data: { error: 'Missing required fields for registration.' } 
              },
            });
          }
        } else {
            reject({ response: { status: 404, data: { error: 'Endpoint not found.' } } });
        }
      }, 1500);
    });
  }
...
const SignUp = () => {
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      setMessage({ 
        text: successMessage || 'Registered successfully! You are now logged in.', 
        type: 'success' 
      });
      ...
      const errorMsg = error.response?.data?.error || 'An unexpected error occurred during registration.';
